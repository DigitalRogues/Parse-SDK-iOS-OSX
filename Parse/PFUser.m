...
+ (void)registerAuthenticationProvider:(id<PFAuthenticationProvider>)authenticationProvider {
    PFParameterAssert(authenticationProvider, @"Authentication provider can't be `nil`.");
    [[self authenticationController] registerAuthenticationProvider:authenticationProvider];
}

+ (void)unregisterAuthenticationProvider:(id<PFAuthenticationProvider>)authenticationProvider {
    PFParameterAssert(authenticationProvider, @"Authentication provider can't be `nil`.");
    [[self authenticationController] unregisterAuthenticationProvider:authenticationProvider];
}
...
+ (BFTask *)_logInWithAuthTypeInBackground:(NSString *)authType authData:(NSDictionary *)authData {
    return [self logInWithAuthTypeInBackground:authType authData:authData];
}

- (BFTask *)_linkWithAuthTypeInBackground:(NSString *)authType authData:(NSDictionary *)newAuthData {
    return [self linkWithAuthTypeInBackground:authType authData:newAuthData];
}

- (BFTask *)_unlinkWithAuthTypeInBackground:(NSString *)authType {
    return [self unlinkWithAuthTypeInBackground:authType];
}
